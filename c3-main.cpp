...
Eigen::Matrix4d NDT(pcl::NormalDistributionsTransform<pcl::PointXYZ, pcl::PointXYZ> ndt, PointCloudT::Ptr source, Pose startingPose, int iterations){
  Eigen::Matrix4f startingPoseTransform = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z).cast<float>();
  ndt.setMaximumIterations(iterations);
  ndt.setInputSource(source);

  PointCloudT::Ptr result (new PointCloudT);
  ndt.align(*result, startingPoseTransform);
...
int main(){
  ...
  lidar_bp.SetAttribute("range", "30");
	...
	lidar->Listen([&new_scan, &lastScanTime, &scanCloud](auto data){
		...
		if(new_scan){
			...
			if(pclCloud.points.size() > 1000){ // CANDO: Can modify this value to get different scan resolutions
  ...
  double poseError = 0;
	...
	while (!viewer->wasStopped())
      ...
      transform = NDT(ndt, filteredCloud, pose, 10);
      ...
      poseError = sqrt( (truePose.position.x - pose.position.x) * (truePose.position.x - pose.position.x) + (truePose.position.y - pose.position.y) * (truePose.position.y - pose.position.y) );
